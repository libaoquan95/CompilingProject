...
StmtBlock         : '{' VarDeclList StmtList '}'   { $$ = new StmtBlock($2, $3); }
                  ...
                  | '{' error '}'  { ReportError::Formatted(&@$, "Can't parse statement block correctly. Attempting to skip."); ReportError::numErrors--; $$ = new StmtBlock(new List<VarDecl*>, new List<Stmt*>); }
                  ;
...
Stmt              : PrintStmt                     { $$ = $1; }
                  ...
                  | SwitchStmt                    { $$ = $1; }
                  | StmtBlock                     { $$ = $1; }
                  | error ';'                     { ReportError::Formatted(&@$, "Cannot parse statement. Skipping statement.");ReportError::numErrors--;  $$ = new EmptyExpr(); }
                  ;
...
SwitchStmt        : T_Switch '(' Expr ')' '{' CaseStatements DefaultStatement '}'  { $$ = new SwitchStmt($3, $6, $7); }
                  ...
                  | T_Switch '(' Expr ')' '{' error '}' { ReportError::Formatted(&@$, "Cannot parse switch statement block. Skipping entire switch statement block."); ReportError::numErrors--; $$ = new SwitchStmt($3, new List<Case *>, NULL); }
                  ;
