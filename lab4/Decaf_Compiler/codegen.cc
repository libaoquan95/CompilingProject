...
void CodeGenerator::Optimize_LA(List<Instruction*>* bb) {
        List<Location*>* op1 = new List<Location*>;
    List<Location*>* op2= new List<Location*>;
    ...
    }else if(bin!=NULL)
    {
        ...
        op1->Append(bin->GetOp1());
        op2->Append(bin->GetOp2());
    }

    }
    for(int i = 0; i < bb->NumElements()-5; ++i)
    {
        int ok=0;
        Assign* agg    = dynamic_cast<Assign*>(bb->Nth(i));
        BinaryOp* binn = dynamic_cast<BinaryOp*>(bb->Nth(i));
        if(agg!=NULL)
        {
            for(int j=0;j<op2->NumElements();j++)
        {

            if(*(agg->dst)==*(op2->Nth(j)))
            {
                ok=1;
                break;
            }
        ...
        for(int j=0;j<op1->NumElements();j++)
        {
            if(*(agg->dst)==*(op1->Nth(j)))
            {
                ok=1;
                break;
            }
